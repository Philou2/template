...
@Component({
  selector: 'readmission-add-edit',
  templateUrl: './readmission-add-edit.component.html',
  styleUrls: ['./readmission-add-edit.component.scss']
})
export class ReadmissionAddEditComponent {
  ...
  create = this.translate.instant('Register');
  ...
  constructor(private fb: FormBuilder,
    ...
    this.readmissionForm = this.fb.group({
      studentlist: '',
      year: '',
      classe: ''
    });
  ...
  ngOnInit() {
    this.readmissionForm.patchValue(this.data);
    if (this.data){
      console.log(this.data);
      console.log(this.data.year);
      const student = this.data.student;
      this.readmissionForm.get('studregistration')?.setValue(this.data);
  ...
  get fc(): any {
    return this.readmissionForm.controls;
  }
  ...
  onFormSubmit(): any {
    ...
    if (this.readmissionForm.valid){
      this.saving = true;
      if (this.data){
        this.studRegistrationService.putGlobal(this.data.id, this.readmissionForm.value).subscribe({
          next: (val: any) => {
          ...
          error: (err: any) =>
          {
            ...
            errors.forEach((v) =>
            {
              if (v.propertyPath === 'name'){
                this.readmissionForm.get('name')?.setErrors({serverError: v.message});
              }
              this.saving = false;
        ...
        console.log(this.readmissionForm.value);
        this.studRegistrationService.postReadmission(this.readmissionForm.value).subscribe({
          next: (val: any) => {
            this.toastr.success('Re-admission created successfully !', 'Success');
            this.activeModal.close();
          },
          complete: () => {
            this.saving = false;
  ...
  findAllStudRegistration(): any{
    this.oldStudentRegistrationService.getStudoldregistrationList().subscribe((data: any) => {
          ...
          this.studregistrations = this.studregistrations.filter((e) => e.studregistration == null && e.status === 'resigned' || e.status === 'dismissed').map((v) => {
            v.id = v['@id'];
            return v;
          });
