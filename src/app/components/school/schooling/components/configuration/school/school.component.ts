...
export class SchoolComponent implements OnInit {
  ...
  constructor(public schoolService: SchoolService, config: NgbModalConfig,
  ...
  getSchoolList(): any {
    this.schoolService.getCollection().subscribe({
      next: (res: any) => {
  ...
  addSchool() {
    const modalRef = this.modalService.open(SchoolAddEditComponent, {
      //  centered: true,
    });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.schoolService.delete(id).subscribe({
          next: (res) => {
            this.toastr.success('School successful deleted', 'Success');
            this.getSchoolList();
          },
          error: (res) => {
            this.loadingIndicator = false;
  ...
  deleteSelectedSchoolRows() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.schoolService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('Schools deleted !', 'Success');
            this.getSchoolList();
            this.remove();
          },
          error: (err: any) => {
