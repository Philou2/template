...
import {TuitionService} from '../../../services/configuration/tuition.service';
// import {ChangeOfMatriculeAddComponent} from './tuition-add/tuition-add.component';
...
export class TuitionComponent implements OnInit {
  ...
  constructor(public tuitionService: TuitionService,
  ...
  getTuitionList(): any {
    this.tuitionService.getCollection().subscribe({
      next: (res: any) => {
        this.tuitions = res['hydra:member'];
  ...
  addTuition() {
    this.router.navigate(['/school/schooling/add-edit-tuition']);
  }

  editTuition(data: any) {
    this.tuitionService.changeTuition(data);
    this.router.navigate(['/school/schooling/add-edit-tuition']);
  }

  deleteTuition(id: number): any {
    Swal.fire({
      title: 'Are you sure?',
      ...
      if (result.isConfirmed) {
        ...
        this.tuitionService.delete(id).subscribe({
          next: (res) => {
            this.toastr.success('Tuition successfully deleted', 'Success');
            this.getTuitionList();
          },
          error: (res) => {
            this.loadingIndicator = false;
  ...
  deleteSelectedTuitionRows() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.tuitionService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('Tuitions deleted !', 'Success');
            this.getTuitionList();
            this.remove();
          },
          error: (err: any) => {
