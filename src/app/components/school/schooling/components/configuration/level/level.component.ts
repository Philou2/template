...
export class LevelComponent implements OnInit {
  ...
  constructor(public levelService: LevelService, config: NgbModalConfig,
  ...
  getLevellist(): any
  {
    this.levelService.getCollection().subscribe({
      next: (res: any) => {
        console.log(res);
        this.leveles = res['hydra:member'];

        console.log(this.leveles);

        // cache our list
        // this.temp = [...res['hydra:member']];
        this.temp = [...res['hydra:member']];

        this.rows = this.leveles;
  ...
  addLevel() {
    const modalRef = this.modalService.open(LevelAddEditComponent);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.levelService.delete(id).subscribe({
          next: (res) => {
            this.toastr.success('Level successful deleted', 'Success');
            this.getLevellist();
          },
          error: (res) => {
            this.loadingIndicator = false;
  ...
  deleteSelectedLevelRows() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.levelService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('Leveles deleted !', 'Success');
            this.getLevellist();
            this.remove();
          },
          error: (err: any) => {
