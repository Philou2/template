...
export class ModuleCategoryComponent implements OnInit {
  ...
  public ModuleCategories!: ModuleCategory[];
  temp = [];
  selected = [];
  ...
  constructor(public moduleCategoryService: ModuleCategoryService, config: NgbModalConfig,
  ...
  getModuleCategorylist(): any
  {
    this.moduleCategoryService.getCollection().subscribe({
      next: (res: any) => {
        console.log(res);
        this.ModuleCategories = res['hydra:member'];

        console.log(this.ModuleCategories);

        // cache our list
        // this.temp = [...res['hydra:member']];
        this.temp = [...res['hydra:member']];

        this.rows = this.ModuleCategories;
  ...
  addModuleCategory() {
    const modalRef = this.modalService.open(ModuleCategoryAddEditComponent);
  ...
  deleteModuleCategory(id: number): any{
    Swal.fire({
      title: 'Are you sure?',
      ...
      if (result.isConfirmed) {
        this.loadingIndicator = true;
        this.moduleCategoryService.delete(id).subscribe({
          next: (res) => {
            this.toastr.success('Module Category successful deleted', 'Success');
            this.getModuleCategorylist();
          },
          error: (res) => {
            this.loadingIndicator = false;
  ...
  deleteSelectedModuleCategoryRows() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.moduleCategoryService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('Module Category deleted !', 'Success');
            this.getModuleCategorylist();
            this.remove();
          },
          error: (err: any) => {
