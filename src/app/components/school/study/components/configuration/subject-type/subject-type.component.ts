...
export class SubjectTypeComponent implements OnInit {
  ...
  constructor(public subjectTypeService: SubjectTypeService, config: NgbModalConfig,
  ...
  getSubjectTypelist(): any
  {
    this.subjectTypeService.getCollection().subscribe({
      next: (res: any) => {
        console.log(res);
        this.SubjectTypes = res['hydra:member'];
  ...
  addSubjectType() {
    const modalRef = this.modalService.open(SubjectTypeAddEditComponent);
  ...
  deleteSubjectType(id: number): any{
    Swal.fire({
      title: 'Are you sure?',
  ...
  deleteSelectedSubjectTypeRows() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.subjectTypeService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('Subject Type deleted !', 'Success');
            this.getSubjectTypelist();
            this.remove();
          },
          error: (err: any) => {
