...
export class RegionComponent implements OnInit {
  ...
  @ViewChild(DatatableComponent) table: DatatableComponent;
    constructor(public regionService: RegionService, config: NgbModalConfig,
  ...
  getRegionlist(): any
  {
    this.regionService.getCollection().subscribe({
      next: (res: any) => {
        console.log(res);
  ...
  addRegion() {
    const modalRef = this.modalService.open(RegionAddEditComponent);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.regionService.delete(id).subscribe({
          next: (res) => {
            this.toastr.success('Region successful deleted', 'Success');
            this.getRegionlist();
          },
          error: (res) => {
            this.loadingIndicator = false;
  ...
  deleteSelectedRegionRows() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.regionService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('Region deleted !', 'Success');
            this.getRegionlist();
            this.remove();
          },
          error: (err: any) => {
