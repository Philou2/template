...
export class DayComponent implements OnInit {
  ...
  @ViewChild(DatatableComponent) table: DatatableComponent;
    constructor(public dayService: DayService, config: NgbModalConfig,
  ...
  getDaylist(): any
  {
    this.dayService.getCollection().subscribe({
      next: (res: any) => {
        console.log(res);
        this.days = res['hydra:member'];
  ...
  addDay() {
    const modalRef = this.modalService.open(DayAddEditComponent);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.dayService.delete(id).subscribe({
          next: (res) => {
            this.toastr.success('day successfully deleted', 'Success');
            this.getDaylist();
          },
          error: (res) => {
            this.loadingIndicator = false;
  ...
  deleteSelectedDayRows() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.dayService.deleteMultiple(ids).subscribe({
          next: (res) => {
            console.log(res);
            this.toastr.success('days deleted !', 'Success');
            this.getDaylist();
            this.remove();
          },
          error: (err: any) => {
